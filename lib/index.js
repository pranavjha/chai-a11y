...
(function() {
    ...
    var loadPlugin = function(cb, eb) {
        ...
        if (!window.auditRunner) {
            ...
            var url = '//raw.githubusercontent.com/dsathyakumar/a11y-auditor/master/dist/a11y-auditor.0-0-14.min.js';
            scriptTag.src = url;
        } else {
    ...
    var stringifyResponse = function(response) {
        ...
        Array.prototype.concat.apply([], Object.keys(response).map(function(ruleKey) {
            ...
            // filter all the FAILed cases & cases which are ERRORs
            return (!item.result && item.severityEnum === 'error');
            ...
            console.log('IsGlobal:        ' + item.isGlobal);
            console.log('');
        });
        return messageCount;
    };
    ...
    var chaiAccessibility = function(chai, utils, modules) {
        ...
        Assertion.addMethod('accessible', function(config, auditConfig) {
            ...
            loadPlugin(function() {
                ...
                // Run the auditor
                var response = window.auditRunner(element.get(0), config.ignore, auditConfig);
